...
main()
{
  ...
  for (i=0;i<N;i++)
  {
    ...
    for (j=0;j<N;j++) 
    {
      ...
      if (i==0.2*N && j>N/4 && j<3*N/4) T[i][j]=-1;
      if (i>0.4*N && i<0.9*N && j == 0.5*N) T[i][j]=1;
  ...
  for (step=0;step<1000000;step++)
  {
    ...
    for (i=0;i<N;i++)
    {
      ...
      if (j==0) color=-8;
      col(color);  
      printf("l3 %f %f %f %f %f %f\n", x, y, T[i][j],x+a, y,T[i+1][j]);
        color=log(0.5*(epshere[i][j]+epshere[i][j+1]))-log(epsref);
      if (i==0) color=-8;
      col(color);
        printf("l3 %f %f %f %f %f %f\n", x, y, T[i][j],x, y+a,T[i][j+1]);
      }
    }
    printf("T -0.2 -0.9\n");
