...
The reason we ask them to solve for velocity, not something else, in this standard kinematics problem is to force them to do something algebraically rather than just putting in numbers right away -- they must solve one equation for time, but it will still have $v_0$ in it.

Choosing the standard coordinate system (where Matt is at the origin), we have
...
They need to then recognize that they are trying the answer the question:

\begin{center}
	\bf ``What value of $v_0$ makes it such that $x=d$ at the same time as $y=0$?
\end{center}

Some students will put the numbers in right away, but they should be discouraged from doing this. Regardless they will need to solve one equation for $t$ and substitute into the other. They can do that in either order.

This gives (solving the y equation first):

$$t = \frac {2 v_0 \sin \theta}{g}$$

and then upon substituting 
