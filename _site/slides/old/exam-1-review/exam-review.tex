...
\item{My office hours today are 1:30-5:30 (in the Clinic)}
\item{Exam review tomorrow: 10AM-4PM; location TBA (waiting on Academic Scheduling)}
...
\EI
  ...
  \frame{\frametitle{\textbf{Problem solving guide: 2D kinematics}}
      \BI
    \item{Draw a picture!}
...
\large
You stand a distance $d$ away from a building and kick a soccer ball at it. The ball's initial velocity is $v_0$, 
directed at an angle $\theta$ above the horizontal. 

\bigskip
\bigskip
\BI
\item{How far above the ground does it hit the building?}
\pause
\item{How fast is it traveling when it hits the building?}
\pause
\item{What direction is it traveling in when it hits?}
\EI
}
...
Practice Exam 3: A hiker on flat ground walks at a constant speed.  She walks north for one hour, walks for
two hours at an angle 30 degrees
south of west, then walks for three hours at an angle 45 degrees
north of
east.
...
\bigskip
\BI
\item{How far must she walk to get back to her starting point?}
\item{Which direction must she walk?}
\EI
}

\frame{\frametitle{\textbf{Example problems: recitation 4.5}}
\large

A swimmer can swim 5 km/hr in still water. She wants to swim directly across a river. However, 
there is a current in the river, with a speed of 2 km/hr. If she swims directly across, she will drift 
downstream due to the current. Thus, in order to get where she wants to go, she needs to angle herself 
upstream.

\BI
\item{There are three interesting vectors in this problem: the velocity of the current, her velocity relative to
the current, and her velocity relative to the shore. How do they relate?}

\pause\bigskip\bigskip\bigskip

\item{At what angle must she point herself to go directly across the river?}

\pause\bigskip\bigskip\bigskip

\item{If the river is 200 m across, how long will it take her?}
\EI
}


\frame{\frametitle{\textbf{Homework 1 Problem 3 (bicycle problem)}}
\large
A bicyclist accelerates at 1 $\rm m/\rm s^2$ for 6 seconds, travels at a constant velocity for 5 seconds, and then applies her brakes, decelerating at 2 $\rm m/\rm s^2$ 
...
\BI
\item{Make $a(t)$ and $v(t)$ graphs.}
\item{How far does she go?}
\item{Make an $x(t)$ graph.}
