...
\BI
\large
\item{Clinic hours today: 1:30-5:30}
\item{Computational project 2 next week -- {\it bring your laptops}}
...
\frame{\frametitle{\textbf{A note on Kepler's law}}
\large
We saw earlier that
...
If we're studying the orbital dynamics of the Earth, then it makes sense to choose some different units for
time and distance.

\BI
\item{Time: measure in years ($T=1$ then)}
\item{Distance: measure in AU ($r=1$ then)}
\EI

This means that $GM_{\rm sun}=4\pi^2$; we will use this next week!}
\end{document}
