...
\EI
\item The first homework is due next Wednesday
\EI
}
...
\frame{\frametitle{\textbf{How fast is the ping-pong ball going when it hits the cans?}}
\Large
\BI
...
\item Find the acceleration that force causes using Newton's second law $F=ma$:
$$
\pi Pr^2 = ma \rightarrow a = \frac{\pi Pr^2}{m}
$$

\item Kinematic relations from before: \color{Red} $x(t) = x_0 + v_0 t + \frac{1}{2}at^2$ and $v(t) = v_0 + at$
\EI
}
...
\large
\begin{enumerate}
\item First: Write down the expressions for $x(t)$ and $v(t)$
\BI
\normalsize
\item $x(t) = \frac{1}{2}at^2$ (since $v_0$ and $x_0$ both equal zero); we will find $a$ later
\item $v(t) = at$ (since $v_0 = 0$)
\EI
\item Second: Ask the right question, in terms of your variables: ``What is $v$ at the time when $x=L$ ?''
\item Third: Do the algebra corresponding to your question.
\BI
\normalsize
\item Set $x=L$: $\frac{1}{2}at^2 = L \rightarrow t=\sqrt \frac{2L}{a}$
\item Find $v$ at that time: $v = at$, so $v = \sqrt{2La}$
\EI
\end{enumerate}


In this problem, we have to go a bit beyond kinematics:
$$
\pi Pr^2 = ma \rightarrow a = \frac{\pi Pr^2}{m}
$$
