...
Homework 7 is due Friday and is quite long; you'll have all the material you need after today's class.
...
\BI
\item Tuesday (today), 2-4 PM
\item Wednesday, 3-6 PM
\EI

\bigskip
\bigskip

Group Exam 3 is this Thursday/Friday in recitation.

\bigskip

Exam 3 is next Tuesday, covering energy and momentum. (I'll give full details Thursday.)

\bigskip

Weekend exam review: {\bf Sunday, 1:30-4:30 PM.}
}
...
\frame{\frametitle{\textbf{A new force: elasticity}}
	
	\Large
	
	Many things in nature stretch and squish.
	\large
	
	\bigskip
	\begin{itemize}
		\item Springs are very stretchy
		\item Some of them are squishy too
		\item Everything will stretch a bit if you pull it hard enough
		\item Guitar strings, rubber bands, steel cables...
		\pause
		\item {\color{Red} This force is called \bf elasticity.}
	\end{itemize}
...
\BI
\item What force does elasticity cause, and what does it depend on?
\pause
\item How much work does the elastic force do?
\pause
\item Can we describe this work using potential energy?
\pause
\EI
}

\frame{\frametitle{\textbf{An experiment with a spring}}

\large

What will this spring's elastic force depend on?

\pause

\bigskip\bigskip

The elastic force depends on:

\BI
\item The physical properties of the thing being stretched (is it a spring or a steel cable?)
\item {\bf The amount it is being stretched}
\EI
...
\begin{center}
	\color{Red} How does this spring's elastic force depend on the amount we stretch it?
\end{center}
}

\frame{\frametitle{\textbf{A simple model of elasticity}}
	\large
	We saw that the elastic force is nearly proportional to the amount of stretch:
	
	\begin{itemize}
		\item $F_{\rm e}$: elastic force (newtons)
		\item $L$: stretched length (meters)
		\item $L_0$: unstretched length (meters)
		\item $k$: proportionality constant (newtons per meter)
	\end{itemize}
...
\centerline{\color{Red}$F_{\rm{elastic}} = -k (L - L_0) \equiv -k \Delta x$}


\vspace{2em}

The minus sign means that the force is in the opposite direction as the stretch/squish.

\small

This is called ``Hooke's law'' after Robert Hooke, an English physicist and contemporary of Newton.
...
\Large
\centerline{$F_{\rm{elastic}} = -k (L - L_0) \equiv -k \Delta x$}
...
\frame{\frametitle{\textbf{The work done by a spring}}
		
	An object of mass $m$ is placed against a spring of spring constant $k$ compressed by an amount $\Delta x_1$. 
	
	\vspace{1em}
	
	When the spring is released, it will propel the object forward.
	
		\vspace{1em}
	When the spring reaches a compression amount $\Delta x_2$, how fast will the object be moving?
	...
	\color{Red}
	\begin{align*}
	KE_1 + W_{\textrm{elastic}, 1 \rightarrow 2} =& KE_2 \\
	W_{\textrm{elastic}, 1 \rightarrow 2} =& \frac{1}{2}mv_2^2
	\end{align*}
...
How much work will the spring do on the object as it moves from position 1 to position 2?

\begin{itemize}
	\color{Green}
	\item A: $W_{\textrm{elastic}} = k \Delta x_1$
	\item B: $W_{\textrm{elastic}} = k \Delta x_2$
	\item C: $W_{\textrm{elastic}} = k \Delta (x_1 - x_2)$
	\item D: $W_{\textrm{elastic}} = k \Delta (x_1 - x_2)^2$
	\item E: Something else
\end{itemize}
\pause

\begin{center}
	\color{Red} \bf E: The force from the spring is changing as the object moves! \\This means we can't do ``work = force times distance moved''
\end{center}
}
...
\begin{center}
	\Large How much work is being done here?
\includegraphics[width=0.5\textwidth]{constant-force.pdf}
...
\begin{center}
	\Large How much work is being done here?
	\includegraphics[width=0.5\textwidth]{two-forces.pdf}
...
\frame{
	\begin{center}
		\Large How do we compute the work done by the spring here?
		\includegraphics[width=0.4\textwidth]{hooke-coarse.pdf}
...
\frame{
	\begin{center}
		\large The work done is the area under the force vs. distance curve...
		\includegraphics[width=0.4\textwidth]{hooke-fine.pdf}
		...
		... just like the change in position is the area under the velocity vs. time curve:
...
\frame{
	\begin{center}
		\large If the spring relaxes all the way, how much work does it do?
		\includegraphics[width=0.4\textwidth]{hooke-fine.pdf}
		...
		\bigskip
				...
				\begin{columns}
						\column{0.25\textwidth}
						\column{0.3\textwidth}
						A: $W = k \Delta x_1$ \\ \vspace{0.5em}
						C: $W = k \Delta x_1^2$
						
						\column{0.4\textwidth}
						B: $W = \frac{1}{2}k \Delta x_1^2$\\ \vspace{0.5em}
						D: $W = \frac{1}{2}k \Delta x_1$
					\end{columns}
...
\frame{
	\begin{center}
		\large What if it doesn't relax all the way, and instead goes from $\Delta x_1$ to $\Delta x_2$?
		\includegraphics[width=0.4\textwidth]{hooke-fine-2.pdf}
...
\frame{
	\begin{center}
		\large What if it doesn't relax all the way, and instead goes from $\Delta x_1$ to $\Delta x_2$?
		\includegraphics[width=0.4\textwidth]{hooke-fine-3.pdf}
		
		\bigskip
		\normalsize
		The red area is the area of the whole triangle, minus the area of the small green triangle:
		\large
		
		$$ W_e = \color{Red}\frac{1}{2}k \Delta x_1^2 \color{Green} - \frac{1}{2}k \Delta x_2^2$$
...
\frame{\frametitle{\textbf{The work done by a spring}}
	...
	We can collect all the ``before'' terms on the left and the ``after'' terms on the right, and identify $\frac{1}{2}k\Delta x^2$ as the {\it elastic potential energy} stored in a spring:

	\begin{align*}
		{\color{Red}\frac{1}{2}k \Delta x_1^2 \color{Green} - \frac{1}{2}k \Delta x_2^2} &= \frac{1}{2}mv_2^2 \\
				{\color{Red}\frac{1}{2}k \Delta x_1^2} &= \frac{1}{2}mv_2^2 \color{Green} + \frac{1}{2}k \Delta x_2^2\\
	\end{align*}

}

\frame{\frametitle{\textbf {The potential energy of a spring}}

\large

\begin{center}

Recall that the gravitational potential energy associated with an object at height $y$ is

$$\Large \color{Red} \text{gravitational potential energy} = mgy$$

\vspace{1in}

Now we see that the elastic potential energy associated with an object of spring constant $k$ stretched or compressed a distance $\Delta x$ is

$$\Large \color{Green} \text{elastic potential energy} = \frac{1}{2}k(\Delta x)^2.$$

\pause


We can just remember this and never need to do this integral again (whew!)
...
\BI
\item{\bf Initial total energy + work done by other forces = final total energy}
...
\frame{\frametitle{\textbf{Power: rate of doing work}}
	...
	\pause
	\centerline{
		$P = \vec F \cdot \vec v = {\color{Red}300~{\rm MW}}$
	}
	...
	\item{Some of that 300 MW of energy dissipated by drag heats up the airplane... (real numbers for a SR-71 Blackbird, the fastest airplane in history)}
	\EI
}
